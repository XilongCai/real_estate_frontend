...
function AddProperty() {
    ...
    function reduceFunction(draft, action) {
        switch (action.type) {
            ...
            case "catchListingTypeChange":
                draft.listingTypeValue = action.listingChosen;
			...
			case "catchListingTypeErrors":
				if (action.listingChosen.length === 0) {
    ...
    // Use effect to change map view depending on borough
    useEffect(() => {
    ...
    useEffect(() => {
        if (state.uploadedPictures[0]) {
            dispatch({type: 'catchPictureChange', pictureChosen: state.uploadedPictures[0]});
        }
	...
	useEffect(() => {
		async function getProfileInfo() {
			try {
				const response = await Axios.get(`https://lbrepapi.com/api/profiles/${globalState.userId}/`);
	...
	useEffect(() => {
		if (state.sendRequest) {
			async function addProperty() {
				...
				formData.append('picture1', state.pictureValue);
				formData.append('seller', globalState.userId);
				console.log(formData);
				try {
					const response = await Axios.post("https://lbrepapi.com/api/listings/create/", formData);
	...
	function submitButtonDisplay() {
		if (globalState.userIsLogged && state.userProfile.agencyName !== null
    ...
    return (
        <div style={{
            ...
            <form onSubmit={formSubmit}>
                ...
                <Grid item container style={{marginTop: '1rem'}}>
                    <TextField 
                        ...
                        label="Title*" 
                        variant="standard" 
                        fullWidth value={state.titleValue} 
				...
				<Grid item container justifyContent="space-between">
					<Grid item xs={5} style={{marginTop: '1rem'}}>
						<TextField 
							...
							onChange={(e) => dispatch({type: 'catchListingTypeChange', listingChosen: e.target.value})}
							onBlur={(e) => dispatch({type: 'catchListingTypeErrors', listingChosen: e.target.value})}
					...
					<Grid item xs={5} style={{marginTop: '1rem'}}>
						<TextField 
							...
							variant="standard" 
							fullWidth value={state.propertyStatusValue} 
				...
				<Grid item container justifyContent="space-between">
					<Grid item xs={5} style={{marginTop: '1rem'}}>
						<TextField 
							...
							fullWidth value={state.rentalFrequencyValue} 
					...
					<Grid item xs={5} style={{marginTop: '1rem'}}>
						<TextField 
							...
							label={priceDisplay()} 
							variant="standard" 
							fullWidth value={state.priceValue} 
                ...
                {state.listingTypeValue === 'Office' ? '' : (
					<Grid item xs={3} container style={{marginTop: '1rem'}}>
						<TextField 
							...
							type="number"
							variant="standard" 
							fullWidth value={state.roomsValue} 
                ...
                <Grid item container justifyContent="space-between">
                    <Grid item xs={5} style={{marginTop: '1rem'}}>
                        <TextField 
                            ...
                            label="Area" 
                            variant="standard" 
                            fullWidth value={state.areaValue} 
                    ...
                    <Grid item xs={5} style={{marginTop: '1rem'}}>
                        <TextField 
                            ...
                            label="Borough" 
                            variant="standard" 
                            fullWidth value={state.boroughValue} 
				...
				<Grid item style={{ marginTop: '1rem'}}>
					{state.latitudeValue && state.longitudeValue ? 
					<Alert severity="success">Your property is located @ {state.latitudeValue},{" "} {state.longitudeValue}</Alert>
                ...
                <Grid item container>
                    <ul>
                        {state.pictureValue ? <li>{state.pictureValue.name}</li> : ""}
                    </ul>
                </Grid>
			...
			<Snackbar
                ...
                message="You have successfully add your property"
                anchorOrigin={{
                    vertical: 'bottom',
                    horizontal: 'center',
                }}
